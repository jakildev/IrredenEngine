...
// IRJob `parallelFor` and checks that:
//
...
TEST_F(ParallelSpawnFixture, WorkerCreateEntityReturnsUniqueIds) {
    ...
    IRJob::parallelFor(0, kCount, 32, [&](int rangeBegin, int rangeEnd) {
        for (int i = rangeBegin; i < rangeEnd; ++i) {
            ids[i] = IREntity::createEntity(
                ParallelSpawnPayload{IRJob::workerId(), i}
            );
        }
    });
...
TEST_F(ParallelSpawnFixture, WorkerSpawnedEntitiesExistAfterFlush) {
    ...
    IRJob::parallelFor(0, kCount, 64, [&](int rangeBegin, int rangeEnd) {
        for (int i = rangeBegin; i < rangeEnd; ++i) {
            ids[i] = IREntity::createEntity(
                ParallelSpawnPayload{IRJob::workerId(), i}
            );
    ...
    // `IRJobFixture.ParallelForRunsOnAtLeastOneWorker` for the same
    // logic on the IRJob side — probabilistic guarantee, not
    // structural).
