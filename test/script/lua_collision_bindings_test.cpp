    ...
    void buildPipeline() {
        m_lua.lua().script(R"lua(
            IRSystem.registerPipeline(IRTime.UPDATE, {
                ...
                IRSystem.systemId(IRSystem.SystemName.COLLISION_DISPATCH_LUA_OVERLAP),
...
TEST_F(LuaCollisionBindingsTest, LayerTableMirrorsCppEnum) {
    sol::state &lua = m_lua.lua();
    EXPECT_EQ(
        lua.script("return IRCollision.Layer.DEFAULT").get<std::uint32_t>(),
        static_cast<std::uint32_t>(IRComponents::COLLISION_LAYER_DEFAULT)
    );
    EXPECT_EQ(
        lua.script("return IRCollision.Layer.NOTE_PLATFORM").get<std::uint32_t>(),
        static_cast<std::uint32_t>(IRComponents::COLLISION_LAYER_NOTE_PLATFORM)
    );
    EXPECT_EQ(
        lua.script("return IRCollision.Layer.PARTICLE").get<std::uint32_t>(),
