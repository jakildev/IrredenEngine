...
**A. SDF composite depth** (`c_shapes_to_trixel.glsl` + `metal/c_shapes_to_trixel.metal`):
**Superseded (2026-06-23b) — see the design doc §Resolution "revised 2026-06-23b".** The first cut snapped the SDF smooth-yaw depth to the **cardinal bracket** (`cardinalYawCosSin(rasterYawCardinalIndex(rasterYaw))`) to make it quadrant-stable. That fixed the at-rest non-cardinal floor drift but **regressed the rotating regime** (the per-axis scatter + detached composite stayed on continuous `visualYaw`, so the cardinal floor desynced from the sliding voxels — clipping + a per-quadrant depth jump). The revised approach **withdraws the cardinal snap** and instead **consolidates all composite depth writers onto one shared continuous-yaw metric** `yawedIsoDistance` (`ir_iso_common.glsl`/`.metal`; CPU twin `IRMath::pos3DtoDistanceYawed`): the SDF smooth path, the per-axis scatter key, and the detached composite offset all derive their occlusion depth from it, so SDF + voxels + detached stay co-sorted at **every** yaw. At a cardinal pose it collapses to un-yawed `x+y+z`, so cardinal frames stay byte-identical. The main-canvas at-rest gather remains cardinal (stored depth feeds AO/shadow reconstruction) → carried to **#1959**.
