...
| Game-side usage | (game-repo; file paths omitted per cross-repo isolation policy) | Primary entity (game-side) | A game-side creation drives unit entities with `C_ShapeDescriptor` via Lua. Exact call sites are in the game repo and must not be cited here. Post-D2 migration tracked in Recommendation step 5. |
