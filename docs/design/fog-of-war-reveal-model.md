...
source hard. The world-canvas kernel is two compile-time variants of one body.
`FOG_TO_TRIXEL` binds the smooth variant only while a live source is smooth,
so a scene with no smooth source pays nothing for the gate. The rule lives in
