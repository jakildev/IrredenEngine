...
| `engine/render/src/shaders/c_shapes_to_trixel_body.glsl` (shared SDF dispatch body) | SDF marches along the (1,1,1) iso depth axis, fixed. | T-056 rotates the world-space SDF query by `visualYaw` (continuous; no integer constraint). |
