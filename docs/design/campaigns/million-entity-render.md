...
| 2026-09-21 | D1.2g, the diagonal's separating experiment, ran and is recorded as a reading: the scatter draw, the relight and the sort switched off one at a time at 45.0° against 44.4° (the first two by their existing kill switches, the sort by a local patch built into `e83ce0cf…`, never committed), two interleaved rounds. Every probe fired in its stage row (scatter 9.4 → 0.4 ms, relight → 0, the sort's row 21.6 → 6.3) and the lane's count is unchanged in every arm, so the probes removed consumers and not the population. The fleet loaded the host mid-run (`host_load_1m` 4.0 to 5.0 in the first 45.0° round, 7.1 to 19 elsewhere in the first round, 6.0 to 9.7 in the second; battery 52% → 35%), so the lower-load round at each pose is read (one dropped cell tripled, five read 1.15 to 1.63× their twin, two within 4%): the envelope at 45.0° falls 7.6 ms with the sort off and 6.0 with the scatter off and rises 0.4 with the relight off (2.4, 1.7 and 0.0 at 44.4°), so of the diagonal's 11.8 ms envelope excess in those cells the sort reads about 5 ms net and the scatter about 4, and the relight nothing the envelope can see although its row is 17 ms. Net and not shares: the arm order is fixed with the load falling, so the sort-off arm always ran quietest; the relight and scatter rows rise when the sort is off; and the earlier Debug, grouped sort-disabled probe read −4.07 ms and is not yet comparable. The sort-off patch is committed beside the reports. The sixteen arms on a quiet host, in reversed or shuffled order, join the #3638 owed list before any share is quoted. Evidence: [`diagonal-pose-cost.md`](../../perf/diagonal-pose-cost.md) § The separating experiment |
| 2026-09-21 | Resync at the top of the #3660 iteration: #3668 (the Codex lane's third PR, `docs: isolate sdf receiver and shadow query failures`, stacked on #3635) is new on this campaign's files. It changes the audit worklist doc, its own captures and this file, and sits on the stack (#3632, #3635) that owns the shadow and cascade kernels, `system_bake_sun_shadow_map.hpp`, `system_entity_canvas_to_framebuffer.hpp` and `ir_render_types.hpp`, which the lane split above already assigns there; no effect on any measurement here. It also edits this file: its `## Contributor lanes` text is that lane's to write, and it rewrites `## Now` and adds a `## Reconciliation checkpoint` section, which are the driver's under the protocol's shared-participation rule. This stack's `## Now` and ledger stand; at merge the driver keeps them and takes #3668's contributor-lanes paragraph, and the conflict is expected |
| 2026-09-21 | D1.2h closed: #3660's mechanism is the sort's encoded span. The first rotated frame of a turn (frame 1 on the cardinal, frame 2 on the set allocated that frame) differs from the settled pose on the unpatched binary at 46.8° (3,564 pixels by 1 to 2) and at 1.2° (3,128 by 1); with a local patch that makes `overflowSortDispatchSpan` return the cap, the first rotated frame at 46.8° is byte-identical to the settled pose (`f42cdb630fb35288`, also D1.2d's pinned capture) and the settled pose is byte-identical between the two binaries, so the patch changes nothing once the lagged count is real. A fresh allocation zeroes `laggedOverflowCount_`, the sort opens its minimum span and the lane's canonical order is incomplete for that one frame; the frame after an unpark carries the pre-park count and is exact. The fix is a lifecycle decision, not this patch (forcing the span every frame encodes every stage for an empty lane): the first live frame after an allocation encodes the full span, or reads its own count back once, and later frames keep the lagged bound. While this slice ran, a fleet worker took #3660 from the agent-approved lane and opened #3671 off master (`overflowSortLaggedBound` returns the cap while the ctrl block's word 0 still holds the allocation's zero seed), so that lane carries the fix, with its own 91.2° control on master's lifecycle and these captures as a second gate. A repeat of the fresh 46.8° arm produced a third frame, 3,484 pixels from the first: with no merge stage encoded the lane is drawn in the order the GPU's atomics appended it, which changes run to run, and the fullspan pair does not. Evidence: [`first-rotated-frame.md`](../../perf/first-rotated-frame.md) |
...
| 2026-09-22 | Resync on D1.2h's second review: two new rows. #3635 (the Codex lane's cascade-receiver coordinates) reads merged, but it merged at 21:20 into `codex/continuous-source-face-lighting`, #3632's branch, an hour after #3632 had squash-merged to master (20:23). Its merge commit `3ecaab5c6` is not an ancestor of `origin/master`, and `test_render_cascade_receiver_space.py` is absent there, so master carries neither the coordinate correction nor the yaw-180 floor regression its body deferred it for (missing/excess 12/266 → 19/356). The earlier rows' reading of #3635 as deferred and open still describes master; the change is stranded work for the Codex lane to re-land off master or drop, and no campaign measurement moves. #3701 (the revoxelized box-shadow oracle, stacked on #3700) corrects that oracle's cardinal half-cell expectation, edits `render-shadow-box-metric.py`, its test, the audit worklist and its own captures, and adds `--probe-box-canvas-size` to IRCanvasStress's shadowbox probe; the default stays 128, so no campaign run changes. It belongs to the SDF stack's lane split below |
...
- 2026-09-22 (lane split after D1.2h's rebase onto master): the Codex SDF
  stack (#3691, #3693, #3694, #3695, #3697, #3698, #3699, #3700, #3701) owns the
  ...
  rows. #3689 and #3690 own the fog files. The Codex lane keeps #3635's
  files (its change is not on master; see the ledger), #3643 keeps the files
...
- **Next:** run the resync first. #3660's fix is #3671's, another lane's PR
  ...
  wait on the Codex lane: its SDF stack from #3691 to #3701, and its
  cascade-receiver change, which is not on master (see the last ledger row).
  D2 has its targets: at 45° the
  ...
  AO are the largest sampled stages at
  every pose, the GPU frame alone is 28 to 30 ms off the band, and the
