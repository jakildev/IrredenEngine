...
| Resampled-cell face ownership | revoxelized detached display against its own inverse-resampled cells; silhouette and per-pixel owner face | `python3 scripts/render-revox-face-metric.py <capture.png> --fixture <cube\|grounded\|lprism> --yaw <degrees>`; [contract and evidence](../design/revoxelized-display-fidelity.md) | render-harness-tests.yml tests the validator; native captures run manually |
