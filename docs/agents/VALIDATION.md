...
| Perf gate | measured frame-time cells against the per-SKU baseline on the `perf-baseline` branch; report-less cells fail as infrastructure errors; the gate's own resolution / exit-mapping / branch-writer logic | `bash scripts/perf/perf_grid_matrix.sh` then `scripts/perf/compare_perf_runs.py`; `python3 scripts/perf/tests/test_baseline_layouts.py`, `scripts/perf/tests/test_baseline_writer.sh` | perf-gate.yml |
