...
You cannot claim a PR an author is actively amending: the scout projection's
`REVIEW_SKIP_PREFIXES` match is the fast path, and `review-claim` refuses at
claim time while a foreign `fleet:amending-*` claim is live. POST-response
arbitration closes the snapshot race between those checks. The PR re-enters
the queue with `fleet:changes-made` once the worker releases the claim.
