...
| `pool-1` … `pool-9` | engine | per dispatch | Generic pane pool — any transient role (worker, reviewers, merger, smoke, epic-steward), routed by the dispatcher; each role's share reserved by its `FLEET_CONCURRENCY_<ROLE>` cap (elastic by default — every launch leaves a free pane for each other pending under-cap role, so a role at cap still takes the panes none of them needs; `FLEET_CAP_MODE=strict` makes caps hard ceilings) |
