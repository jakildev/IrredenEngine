...
def measure(path: Path, unblocked: bool) -> bool:
    ...
    # Screen coordinates use yaw pi and a 4x2 isometric scale at zoom 1.
    samples = [
        ("blocked-floor", (1280, 718), top_factor if unblocked else AMBIENT),
        ("outside-floor", (1200, 710), top_factor),
    ]
    if not unblocked:
...
def main() -> None:
    ...
    results = [measure(path, args.unblocked) for path in args.images]
