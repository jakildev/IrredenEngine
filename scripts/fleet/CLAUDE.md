...
- **An ingest round-trip's candidate set is captured above the section
  ...
  claimed issue; capture into a flat `tasks.plan_gated` list inside the loop,
  above the `continue`s, from the raw issue list. Pin the placement with a
  test asserting the row is in the candidate list **and** absent from every
  section (`tests/test_scout_task_queue_plan_gated.py`); asserting only the
  former passes with the capture moved back below a `continue`.
