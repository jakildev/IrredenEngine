...
- **A suite whose subject under test is missing must not report success.**
  ...
  **not** `fleet-tests.yml`-scoped: `tests/test_workflow_paths_sync.sh`
  ...
  #3187), and `fleet_test_subjects.py`'s F2 asserts the same population
  against `OUT_OF_TREE_SUBJECTS`, which T4 never reads. T6 in that suite
  asserts the two derivations agree, so the shared glob has one meaning.
