...
# Drivers that launch a demo through fleet-run. The macOS Seatbelt profile
# grants no WindowServer lookup, so a demo started from a sandboxed driver
# discovers no display and idles until the fleet-run watchdog kills it. An
# allow rule runs the driver outside the sandbox, as it already runs fleet-run.
DISPLAY_VALIDATORS = (
...
def rules(worktree, role):
    ...
    for script in DISPLAY_VALIDATORS:
        runners = (["bash"], []) if script.endswith(".sh") else (["python3"],)
