#!/usr/bin/env bash
# regression: fleet-claim's duplicate-open-PR guard (cmd_claim's `gh pr list
# --repo ... --state open --json number,headRefName,body`) passes `--limit
# 300`, so an issue's in-flight PR is found even when `gh pr list`'s own
# default of 30 would otherwise truncate it out of the newest-30 window and
# let a duplicate claim through.
#
...
# fixed call site, so this suite fails on any `gh pr list` call site that
# omits `--limit`.
#
...
# 31-PR pool (30 filler PRs plus one headRefName claude/9101-dup — the ONLY
# PR that matches issue 9101; nothing matches issue 9102) and truncates it
# exactly like real `gh pr list`: `--limit N` returns the first N, no
# `--limit` (or N<31) returns only the 30 filler PRs, hiding the match.
...
# --- T1: issue 9101's own PR sits outside the newest-30 window -------------
...
# no PR anywhere in the (same, full) 31-PR set matches issue 9102 -> claim
# succeeds.
