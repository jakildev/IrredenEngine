...
#   - the shutdown sentinel short-circuits (rc=1)
#   - an uninterrupted sleep elapses and returns 0
#   - the function's parameter list carries no wake-suppression floor, and
#     every call site passes an arity it accepts (1 or 2 args) — the only arm
#     that reads source text rather than observing behaviour
#
# This file lives under scripts/, which the fleet's acceptance greps sweep for
# the identifiers T4 asserts absent; spelling them here would make such a grep
# nonzero. T4 therefore assembles its needles from fragments and controls them
# against a fixture built to carry the shape it asserts absent.
#
...
# No wake-suppression window guards that, so it holds however recently the
# previous wake occurred.
...
# --- T2: the shutdown sentinel short-circuits -------------------------------
...
# --- T4: no wake-suppression floor parameter; call sites match the arity -----
# T1-T3 are behaviour statements that a floor defaulting to 0 would satisfy
# just as well, so they cannot detect one; this arm reads the shipped source,
# which is the only way to.
#
# The needles are assembled from fragments so this file does not itself carry
# the literals T4 asserts absent — see the header. The names still reach the
# test output, just not the source text.
FRAG_FLOOR=floor
FRAG_BACKOFF=BACKOFF
ABSENT_PARAM="min_$FRAG_FLOOR"                              # parameter needle
ABSENT_VARS="FLEET_MIN_$FRAG_BACKOFF|MIN_${FRAG_BACKOFF}_SECONDS"  # config-var needles
echo "T4: $ABSENT_PARAM is absent and all call sites pass 1 or 2 args"
...
# match them against a fixture built to carry that shape first.
FIXTURE="$TMPROOT/absent-shape"
{
    printf 'interruptible_sleep() {\n    local %s="${3:-0}"\n}\n' "$ABSENT_PARAM"
    printf 'FLEET_MIN_%s=1800\nMIN_%s_SECONDS=1800\n' "$FRAG_BACKOFF" "$FRAG_BACKOFF"
} > "$FIXTURE"
[[ "$(grep -cF "$ABSENT_PARAM" "$FIXTURE" || true)" -gt 0 ]] \
...
[[ "$(grep -cE "$ABSENT_VARS" "$FIXTURE" || true)" -gt 0 ]] \
...
assert_absent "$BODY" "$ABSENT_PARAM" \
    "the shipped function body has no $ABSENT_PARAM parameter"
assert_eq "$(grep -cE "$ABSENT_VARS" "$BABYSIT" || true)" "0" \
    "fleet-babysit does not name the cooldown config vars"
# Second control, on the file rather than the needles: the same grep form must
# find a name that IS in fleet-babysit. Without it, an unreadable path
# would score the absence assertion as satisfied.
[[ "$(grep -c 'LONG_BACKOFF_SECONDS' "$BABYSIT" || true)" -gt 0 ]] \
    && ok "grep control: the same form finds LONG_BACKOFF_SECONDS" \
...
while IFS= read -r line; do
    ...
    if [[ "$n" -ge 1 && "$n" -le 2 ]]; then
        ok "call site :$lineno passes $n arg(s) — within the accepted arity"
    else
