...
build_haystack() {
    ...
    while (( kib <= 8192 )); do
        ...
        set +e
        ( piped_form_has "$hay" "$NEEDLE" ) 2>/dev/null
        rc=$?
        set -e
        if [[ "$rc" -ne 0 ]]; then
            HAYSTACK="$hay"
...
# The writer's failure reaches the pipeline as 141 where the shell dies of
# SIGPIPE and as 1 where bash's printf builtin reports EPIPE itself, so the
# assertion is on the part that holds either way: the status is not grep's.
# Pinning the literal 141 would encode one platform's spelling of the bug into
# the guard against it.
echo "    (writer status on this host: $PIPED_RC)"
piped_verdict=$([[ "$PIPED_RC" -eq 0 ]] && echo "grep-verdict" || echo "writer-error")
assert_eq "$piped_verdict" "writer-error" \
    "the piped form's status comes from the writer, not from grep's match"
