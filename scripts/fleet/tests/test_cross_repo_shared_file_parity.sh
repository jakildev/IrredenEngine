...
# vacuous pass; this is why the check cannot simply be a CI job.
#
...
#
# Add a third shared file by adding one entry to CONTRACT_FILES — no other
# change needed.
...
# The declared contract mirrors .github/workflows/auto-rereview.yml's header
...
# "no drift" always means "compared every declared file". The loop uses the
# guarded expansion so this FAIL reaches summarize rather than aborting on
# an empty [@] under set -u (no bash floor is declared here).
...
for path in "${CONTRACT_FILES[@]+"${CONTRACT_FILES[@]}"}"; do
    ...
    # basename would otherwise share one pair of temp blobs, breaking the
    # "add an entry, no other change needed" guarantee for shared basenames.
    slug="${path//\//_}"
