#!/usr/bin/env bash
# Test the fleet-queue-ingest retract pass.
#
# The three planning queue-blocks are enforced only at ingest STAMP time:
# nothing retracts an already-stamped fleet:queued when a gate lands
# afterwards. A gate arriving after queuing holds fleet-queue-ingest but NOT
# worker pickup — the issue stays in the scout's pickable set and
# `fleet-claim claim` can grant it. Ingest strips fleet:queued from every
# retract candidate, and NEVER touches the gate label itself: the
# human/reviewer owns that, and clearing it re-queues the issue through the
# normal add path.
#
# Two gates, not three: human:review-plan is retired and inert
...
#     can land on a claimed issue exactly this way.
#
...
# retract_issues:
#   issue 780 queued + fleet:needs-plan                → retract
#   issue 781 queued + fleet:plan-review               → retract  (constraint-1 arm)
#   issue 782 queued + fleet:plan-review + a claim     → retract, claim untouched
#                                                   (constraint-2 arm)
#   issue 783 gate already cleared live                → skip, no edit
#   issue 784 fleet:queued already gone live           → skip, no edit
# pending_issues:
#   issue 785 approved, planned, gate-free             → regression arm: still queued
cat > "$PROJ" <<'JSON'
{"pending_issues":[
