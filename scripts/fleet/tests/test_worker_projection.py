...
class ActiveClaimBarsConflictResolutionPickup(unittest.TestCase):
    ...
    ReviewClaimBarsWorkerFeedbackPickup below closes the
    fleet:reviewing-* -> fleet:amending-* direction. The conflict-resolution
    lane needs its own bar: fleet:reviewing-* and fleet:resolving-* are
    DISJOINT claim namespaces, so _semantic_conflict_claimable's own
    fleet:resolving- test (and _acquire_label_on's lex-min tie-break, which
    filters to labels startswith its own prefix) is structurally blind to a
    live review claim.
