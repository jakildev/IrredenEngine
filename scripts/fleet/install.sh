...
FLEET_STALE_SWEEP_SRC="$SCRIPT_DIR/fleet-stale-sweep"
FLEET_STALE_SWEEP_DEST="$HOME/bin/fleet-stale-sweep"
...
# off, fix it here.
for src in "$FLEET_UP_SRC" "$FLEET_DOWN_SRC" "$FLEET_CLAIM_SRC" "$FLEET_COMMON_SRC" "$FLEET_BUILD_SRC" "$FLEET_DEBUG_SRC" "$FLEET_RUN_SRC" "$FLEET_RUN_TARGETS_SRC" "$FLEET_HELP_SRC" "$FLEET_BABYSIT_SRC" "$FLEET_LABELS_SRC" "$FLEET_HEARTBEAT_SRC" "$FLEET_STREAM_SRC" "$FLEET_SCOUT_SRC" "$FLEET_FEEDBACK_SRC" "$FLEET_BUSY_SRC" "$FLEET_PR_SRC" "$FLEET_PR_CLEAR_SRC" "$FLEET_PR_DETACH_SRC" "$FLEET_PR_AMEND_SRC" "$FLEET_ISSUE_SRC" "$FLEET_DISPATCHER_SRC" "$FLEET_DISPATCH_WRAP_SRC" "$FLEET_GATE_STATUS_SRC" "$FLEET_TASKS_RENDER_SRC" "$FLEET_QUEUE_TICK_SRC" "$FLEET_QUEUE_INGEST_SRC" "$FLEET_QUEUE_LIST_SRC" "$FLEET_STALE_SWEEP_SRC" "$FLEET_RECONCILE_AMENDMENTS_SRC" "$FLEET_ITERATION_SUMMARY_SRC" "$FLEET_EDIT_SRC" "$WITNESS_SRC"; do
    if [[ -f "$src" && ! -x "$src" ]]; then
        chmod +x "$src"
    fi
done
...
if [[ -f "$FLEET_STALE_SWEEP_SRC" ]]; then
    ln -sf "$FLEET_STALE_SWEEP_SRC" "$FLEET_STALE_SWEEP_DEST"
    echo "symlinked $FLEET_STALE_SWEEP_DEST -> $FLEET_STALE_SWEEP_SRC"
fi
