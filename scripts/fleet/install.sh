...
FLEET_CODEX_SRC="$SCRIPT_DIR/fleet-codex"
FLEET_CODEX_DEST="$HOME/bin/fleet-codex"
FLEET_RUNTIME_SRC="$SCRIPT_DIR/fleet-runtime"
FLEET_RUNTIME_DEST="$HOME/bin/fleet-runtime"
...
FLEET_HEALTH_SRC="$SCRIPT_DIR/fleet-health"
FLEET_HEALTH_DEST="$HOME/bin/fleet-health"
...
FLEET_TRIAGE_SWEEP_SRC="$SCRIPT_DIR/fleet-triage-sweep"
FLEET_TRIAGE_SWEEP_DEST="$HOME/bin/fleet-triage-sweep"
...
FLEET_RULES_SWEEP_SRC="$SCRIPT_DIR/fleet-rules-sweep"
FLEET_RULES_SWEEP_DEST="$HOME/bin/fleet-rules-sweep"
FLEET_ASSERT_WT_SRC="$SCRIPT_DIR/fleet-assert-worktree"
FLEET_ASSERT_WT_DEST="$HOME/bin/fleet-assert-worktree"
FLEET_START_NEXT_TASK_SRC="$SCRIPT_DIR/fleet-start-next-task"
FLEET_START_NEXT_TASK_DEST="$HOME/bin/fleet-start-next-task"
...
FLEET_QUEUE_BACKFILL_MODEL_LABELS_DEST="$HOME/bin/fleet-queue-backfill-model-labels"
...
if [[ -f "$FLEET_CODEX_SRC" ]]; then
    ln -sf "$FLEET_CODEX_SRC" "$FLEET_CODEX_DEST"
fi
if [[ -f "$FLEET_RUNTIME_SRC" ]]; then
    ln -sf "$FLEET_RUNTIME_SRC" "$FLEET_RUNTIME_DEST"
fi
...
if [[ -f "$FLEET_POSITIVE_CONTROL_SRC" ]]; then
    ln -sf "$FLEET_POSITIVE_CONTROL_SRC" "$FLEET_POSITIVE_CONTROL_DEST"
    echo "symlinked $FLEET_POSITIVE_CONTROL_DEST -> $FLEET_POSITIVE_CONTROL_SRC"
fi
...
if [[ -f "$FLEET_HEALTH_SRC" ]]; then
    ln -sf "$FLEET_HEALTH_SRC" "$FLEET_HEALTH_DEST"
    echo "symlinked $FLEET_HEALTH_DEST -> $FLEET_HEALTH_SRC"
fi
...
if [[ -f "$FLEET_PR_BODY_LINT_SRC" ]]; then
    ln -sf "$FLEET_PR_BODY_LINT_SRC" "$FLEET_PR_BODY_LINT_DEST"
...
if [[ -f "$FLEET_TRIAGE_SWEEP_SRC" ]]; then
    ln -sf "$FLEET_TRIAGE_SWEEP_SRC" "$FLEET_TRIAGE_SWEEP_DEST"
    echo "symlinked $FLEET_TRIAGE_SWEEP_DEST -> $FLEET_TRIAGE_SWEEP_SRC"
fi
...
if [[ -f "$FLEET_RULES_SWEEP_SRC" ]]; then
    ln -sf "$FLEET_RULES_SWEEP_SRC" "$FLEET_RULES_SWEEP_DEST"
...
if [[ -f "$FLEET_START_NEXT_TASK_SRC" ]]; then
    ln -sf "$FLEET_START_NEXT_TASK_SRC" "$FLEET_START_NEXT_TASK_DEST"
    echo "symlinked $FLEET_START_NEXT_TASK_DEST -> $FLEET_START_NEXT_TASK_SRC"
fi
...
if [[ -f "$REVIEW_FLEET_FEEDBACK_SRC" ]]; then
    ln -sf "$REVIEW_FLEET_FEEDBACK_SRC" "$REVIEW_FLEET_FEEDBACK_DEST"
    echo "symlinked $REVIEW_FLEET_FEEDBACK_DEST -> $REVIEW_FLEET_FEEDBACK_SRC"
fi

if [[ -f "$FLEET_QUEUE_BACKFILL_MODEL_LABELS_SRC" ]]; then
    ln -sf "$FLEET_QUEUE_BACKFILL_MODEL_LABELS_SRC" "$FLEET_QUEUE_BACKFILL_MODEL_LABELS_DEST"
    echo "symlinked $FLEET_QUEUE_BACKFILL_MODEL_LABELS_DEST -> $FLEET_QUEUE_BACKFILL_MODEL_LABELS_SRC"
fi
