...
# CPU split: this box is shared with the WSL2 fleet (same physical machine), so
# default to HALF the threads. For a Windows-only session bump FLEET_CPU_BUDGET
# to nproc (and cap the WSL host.toml to match when co-running).
FLEET_CPU_BUDGET="${FLEET_CPU_BUDGET:-16}"
...
# Shared with the WSL2 fleet on the same box; budget = half the threads by default.
[cpu]
