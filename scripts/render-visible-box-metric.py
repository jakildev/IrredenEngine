...
def expected_polygon(image, yaw):
    ...
    points = []
    for x, y, z in itertools.product(*[(-h - 0.5, h + 0.5) for h in BOX_HALF_CENTER_SPAN]):
        px, py = project((x, y, z + BOX_Z - FLOOR_TOP), yaw)
        points.append((center[0] + px * scale[0], center[1] + py * scale[1]))
    return convex_hull(points)


def measure(path: Path, yaw: float, overlay: Path | None, placement_only=False) -> bool:
    with Image.open(path) as opened:
        image = opened.convert("RGB")
    polygon = expected_polygon(image, math.radians(yaw))
