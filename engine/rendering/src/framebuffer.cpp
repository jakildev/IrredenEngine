 ...
 * File: \irreden-engine\engine\src\rendering\framebuffer.cpp
...
namespace IRRender {
    ...
    void Framebuffer::checkSuccess() {
        GLenum status = ENG_API->glCheckNamedFramebufferStatus(
            m_id,
            GL_FRAMEBUFFER
        );

        IRProfile::engAssert(status == GL_FRAMEBUFFER_COMPLETE,
