...
class LuaScript {
  public:
    ...
    template <typename... Components> void registerCreateEntityBatchFunction(const char *funcName) {
        static_assert(
            sizeof...(Components) > 0,
            "LuaScript::registerCreateEntityBatchFunction requires at least one component"
        );
        if (!m_lua["IREntity"].valid()) {
            m_lua["IREntity"] = m_lua.create_table();
        }
  ...
  private: //----------------------------------------------------------------
    ...
    template <typename> using ProtectedFunctionFor = sol::protected_function;

    template <typename... Components> auto wrapCreateEntityBatchWithFunctions() {
        IR_LOG_INFO("Creating entity batch with {} components", sizeof...(Components));
        return [this](IRMath::ivec3 partitions, ProtectedFunctionFor<Components>... functions) {
            std::vector<IREntity::EntityId> entities = IREntity::createEntityBatchWithFunctions_Ext(
                partitions,
                {},
