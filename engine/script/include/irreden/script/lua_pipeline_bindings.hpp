...
#define IR_BIND_SYS(name) t[#name] = static_cast<lua_Integer>(IRSystem::name)
    ...
    IR_BIND_SYS(GLOBAL_POSITION_3D);
    IR_BIND_SYS(APPLY_POSITION_OFFSET);
    IR_BIND_SYS(VOXEL_SQUASH_STRETCH);
    IR_BIND_SYS(UPDATE_VOXEL_SET_CHILDREN);
    IR_BIND_SYS(VOXEL_SCENE);
    ...
    IR_BIND_SYS(VOXEL_TO_TRIXEL_STAGE_2);
    ...
    IR_BIND_SYS(SCREEN_SPACE_RESIDUAL_ROTATE);
    ...
    IR_BIND_SYS(BUILD_OCCUPANCY_GRID);
