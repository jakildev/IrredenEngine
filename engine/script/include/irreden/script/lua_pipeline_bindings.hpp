...
// Lua bindings for IRSystem::registerPipeline + the SystemName enum.
...
#define IR_BIND_SYS(name) t[#name] = static_cast<lua_Integer>(IRSystem::name)
    ...
    IR_BIND_SYS(REBUILD_GRID_VOXELS_IMPLICIT);
...
inline void bindRegisterPipelineAndSystemId(
    ...
    // `accumulatedDeltaTime`). `sysId` is any SystemId (from
