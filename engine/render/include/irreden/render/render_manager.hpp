...
class RenderManager {
  public:
    ...
    // a settled camera whose pan/zoom moved, and the rotation-start edge,
    // #2669 — held otherwise). Depth 0 — before the first derive and whenever
    // the center pixel reads background — is the pre-#2547 point exactly.
  ...
  private:
    ...
    // Depth-aware default-pivot latch (#2547) and its update policy (#2669).
    // Only the iso DEPTH is latched — the focus POINT is re-derived from the
