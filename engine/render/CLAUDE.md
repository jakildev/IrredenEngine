...
**Silhouette-riser exception for authored rotated GRID content.**
`VoxelReserved::kRotatedEmit` (reserved bit 2, set by
`REBUILD_GRID_VOXELS`) enables opposite-polarity risers and dual emission
when the canvas is not revoxelized. This preserves the rotated GRID
footprint at grazing camera angles.

Revoxelized detached occupancy is already camera-aligned and uses the
strict visible-face triplet. Its opposite-polarity faces are back-facing;
emitting them adds invalid surface fragments even for one unrotated voxel.
Different camera-facing normals along voxel staircases remain valid and
are preserved. See
[`detached-face-normals.md`](../../docs/design/detached-face-normals.md)
for the diagnostic and its limits.
