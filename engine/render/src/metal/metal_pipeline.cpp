...
MTL::Size threadgroupSizeForFunctionName(const std::string &functionName) {
    ...
    if (functionName == "c_voxel_visibility_compact" ||
        ...
        functionName == "c_fog_overflow_faces" ||
        functionName == "c_bake_voxel_sun_faces") {
        return MTL::Size(64, 1, 1);
    }
    // Overflow canonical sort uses 256 threads over a 2048-element fused slab
    // (8 elements/thread). MUST match the GLSL local_size_x literal and
    ...
    if (functionName == "c_fog_to_trixel" || functionName == "c_fog_to_trixel_los_smooth") {
        return MTL::Size(16, 16, 1);
    }
    if (functionName == "c_build_distance_hiz") {
        return MTL::Size(16, 16, 1);
    }
    if (functionName == "c_chunk_occlusion_cull") {
        return MTL::Size(64, 1, 1);
    }
    if (functionName == "c_compute_sun_shadow" || functionName == "c_compute_sun_shadow_shapes") {
        return MTL::Size(16, 16, 1);
    }
    ...
    // Per-axis empty-cell compaction uses one thread per canvas cell, dispatched
    // 2D over the axis grid (matches local_size_{x,y} = 16 in
    ...
    // Per-axis dispatch-dimension finalization uses one thread per axis,
    // dispatched as (kAxisCount, 1, 1). This is an explicit entry rather than
    // an implicit coincidence with the fallback below.
...
// params UBO on every encode after the first distance-image bind. The fill then
// reads distance-clear words as its params and authors nothing. Like
// threadgroupSizeForFunctionName above — whose membership is enforced by
// cmake/run_metal_kernel_registry_check.cmake — this list is
// hand-maintained but not un-checked:
// cmake/run_metal_scratch_consumer_check.cmake derives the expected
...
// such parameter, since the bind would then land on
