...
std::string ShaderStage::readFileAsString(const char *filepath) {
    ...
    try {
        ...
        IRE_LOG_FATAL("std::ifstream::failure exception reading file: {}, {}", filepath, e.what());
...
void ShaderStage::checkSuccess() {
    ...
    if (!success) {
        ...
        ENG_API->glGetShaderiv(m_handle, GL_INFO_LOG_LENGTH, &infoLogLength);
        std::vector<GLchar> infoLog(infoLogLength);
        ENG_API->glGetShaderInfoLog(m_handle, infoLogLength, NULL, infoLog.data());
        IRE_LOG_FATAL(infoLog.data());
        IR_ASSERT(false, "Shader stage compilation failed.");
    }
}

ShaderProgram::ShaderProgram(const std::vector<GLuint> &stages)
...
void ShaderProgram::checkSuccess() {
    ...
    if (!success) {
        ...
        std::vector<GLchar> infoLog(infoLogLength);
        ENG_API->glGetProgramInfoLog(m_handle, 512, NULL, infoLog.data());
        IRE_LOG_FATAL(infoLog.data());
