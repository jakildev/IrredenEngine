...
//    (`dest_.y³` cells, center `dest_.z`). Forward scatter (mode 0 under
//    rotation) is NOT surjective onto the rotated lattice, so some covered dest
//    cells would get no source voxel (holes). Inverse resampling dispatches over
//    the DEST lattice and pulls: dest cell `c` inverse-maps to source cell
//    `roundHalfUp(R⁻¹·(c + anchor) - anchor)` (the half-cell-anchored map); if
//    that source cell is occupied (in the per-pool source grid, binding 9) the
//    thread authors `position[slot]=c + anchor`, `color[slot]=srcColor`, and sets
//    the per-slot active bit. Surjective by construction → hole-free at every
...
// adjacency (the GPU twin of REBUILD_GRID_VOXELS' CPU mask), so stage 1/2 gate
// the re-voxelize emit on `faceIsExposed` exactly like the GRID path.
...
layout(std140, binding = 16) uniform RevoxelizeParams {
    ...
    vec4 anchor_;         // xyz = half-cell anchor: solid point = cell + anchor
};
...
void main() {
    ...
    // decode window covers them at zero dispatch growth.
    int side = dest_.y;
    int center = dest_.z;
    ivec3 d = ivec3(
        int(slot) % side,
