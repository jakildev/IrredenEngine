...
// Mirrors shaders/c_update_voxel_positions.glsl byte-for-byte. Sentinel-slot
// (kVoxelTransformStatic) voxels are left untouched so the CPU-direct path
// owns them. The per-voxel transform slot is bit-packed into the
// local-position .w lane. The threadgroup grid + linear index reconstruction
