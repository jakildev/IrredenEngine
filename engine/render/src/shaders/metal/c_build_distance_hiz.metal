...
// Mirrors shaders/c_build_distance_hiz.glsl. The max runs over raw encoded
// ints (larger encoded value <=> farther; the 65535 background sentinel is the
// largest, so a footprint still seeing background never occludes).
// Ceil-division destination sizing (makeHiZMipChain) makes the clamp on the +1
// taps re-read only an in-bounds texel.
