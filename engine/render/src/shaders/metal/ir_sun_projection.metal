...
constant int kSunPcfTapMin = -1;
constant int kSunPcfTapMax = 2;
...
constant int kSunCascadeCasterMarginTexels = 2;
...
inline bool sunCascadeKernelInterior(float2 sunUV, float2 origin, float2 texelSz) {
    ...
    // drift (#2321).
    int2 lo = base + int2(kSunPcfTapMin - kSunCascadeCasterMarginTexels);
    int2 hi = base + int2(kSunPcfTapMax + kSunCascadeCasterMarginTexels);
