...
// Mirrors shaders/ir_sun_projection.glsl. Sun-space projection shared by the
// caster bake (c_bake_sun_shadow_map) and the receiver lookup
// (ir_sun_shadow_sample), so caster depth and receiver lookup use one basis and
// cannot drift apart. CPU twin: IRMath::sunSpaceProject.
...
float sunCascadeReceiverDepth(float3 worldPosition, float3 worldDepthAxis) {
...
inline uint packSunSurfaceDepth(float sunZ) {
    return packSunDepth(sunZ, int2(-8));
}
...
// 0x88 is analytic geometry; 0x89 identifies rigid source faces in the query index.
inline uint sunVoxelFaceMarker(int faceId, int basis) {
    if (basis == 2) return 0x89u;
