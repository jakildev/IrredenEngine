...
// Mirrors shaders/c_fog_to_trixel_body.glsl: one kernel, main-canvas and
// per-axis routes, shaded by the shared reveal model (ir_fog_common.metal). A
// fully revealed pixel is never read or rewritten. The wrapper defines
// IR_FOG_KERNEL_NAME and IR_FOG_LOS_SMOOTH (the GLSL twin documents the pair).
...
kernel void IR_FOG_KERNEL_NAME(
    ...
    const bool losSmooth = IR_FOG_LOS_SMOOTH != 0 && frameData.perAxisRoute == 0;
