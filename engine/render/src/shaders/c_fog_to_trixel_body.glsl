...
//
// Include-BODY: the wrapper defines IR_FOG_LOS_SMOOTH. 0 (c_fog_to_trixel)
// gates every source hard, and the smooth gate's taps, face recovery and
// visibility math compile out of the reveal loop; 1
// (c_fog_to_trixel_los_smooth) carries the smooth gate on the main canvas.
// FOG_TO_TRIXEL picks the smooth variant only while a smooth source is live.
...
void main() {
    ...
    // its voxel from the raster, and only when a smooth source reads it. A
    // compile-time false folds the smooth arm out of fogRevealSample.
    const bool losSmooth = IR_FOG_LOS_SMOOTH != 0 && perAxisRoute == 0;
