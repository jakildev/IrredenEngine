...
//
// Include-FRAGMENT: the wrapper defines IR_FOG_LOS_SMOOTH. c_fog_to_trixel (0)
// gates every line-of-sight source hard and serves both routes;
// c_fog_to_trixel_smooth (1) adds the smooth gate and is dispatched only on the
// main canvas, only while a gated source is smooth.
...
#if IR_FOG_LOS_SMOOTH
    // A vertical face recovers its voxel from the raster, and only when a
    // smooth source reads it.
    if ((faceId >> 1) != kZFace && fogLosSmoothSampleNeeded(fogWholeBody)) {
