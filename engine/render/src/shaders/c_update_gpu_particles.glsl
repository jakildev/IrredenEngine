...
// Spawning is CPU-side and batched: `C_GPUParticlePool::writeSlot` queues
// new particles into a per-frame pending list; the host system flushes
// them via `pool.flushPendingSpawns()` immediately before dispatching this
// shader, coalescing into one `subData` per contiguous run of touched
// slot indices. Between mutations the GPU is the source of truth — the
// CPU never reads back. A future phase adds a GPU spawn pass for
// compute-driven emitters.
