...
void main() {
    ...
    ivec2 pixel;
    float sourceAO = 1.0;
    vec3 sourceViewCenter = vec3(0.0);
    ...
    if (worldReceive && sourceMode) {
        worldReceivePos = rotateByQuat(sourceViewCenter, detachedViewToWorld)
...
#if IR_SHAPE_RECEIVER
    worldNormal = receiverFaceNormal(imageLoad(canvasSunShadow, pixel).a, worldNormal);
#endif
