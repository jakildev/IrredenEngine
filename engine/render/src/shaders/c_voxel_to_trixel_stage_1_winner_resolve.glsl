...
// Stage-1 CARDINAL WINNER-ELECTION dispatch. A thin wrapper that
...
// equal-key faces — the per-axis election applied to the single-canvas store,
// whose (iso pixel, depth) key stops being a bijection of live voxels once
// independently displaced voxels round into the same cell.
// Dispatched over indirect struct 0 ONLY (feeder-won pixels are never
// colour-tapped — the shadow-feeder margin), between the stage-1 stores and
// stage 2, and only when the ticking pool's storeTiesPossible_ flag is set.
// The visible twin is c_voxel_to_trixel_stage_1.glsl. Includes come BEFORE the
