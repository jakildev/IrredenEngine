...
// Hi-Z (hierarchical max-depth) downsample for voxel occlusion culling. One
// dispatch per mip level: reads the previous level's distance image and writes
// each destination texel the MAX (farthest) of its (up to) 2x2 source
// footprint. Conceptual level 0 is the canvas trixelDistances; this pass
// produces the downsampled levels on C_TriangleCanvasTextures::hiZMips_.
...
// footprint". The chunk-occlusion pre-pass culls a pool-chunk only when its
// NEAREST depth is strictly behind that max — i.e. closer geometry covers the
// whole footprint. The empty/background sentinel (65535) is the largest
// encoded value, so any footprint still seeing background keeps the max at
// 65535 = "never occlude" — the conservative direction (a false positive is a
// visible hole; a false negative is only lost savings).
//
// The raw R32I encoded distance is max'd directly: the main-canvas encoding
// packs depth in bits [31:3] (flip at [2], face slot in [1:0]), so larger
// encoded value <=> farther, and an integer max over encoded values is a
// faithful max-depth pyramid. The chunk-occlusion pass compares raw encoded
