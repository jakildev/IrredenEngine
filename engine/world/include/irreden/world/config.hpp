...
class WorldConfig {
  public:
    WorldConfig(const char *luaConfigFile) : m_lua{luaConfigFile}, m_config{} {
        m_config.addEntry("init_window_width",
                          std::make_unique<IRScript::LuaValue<IRScript::LuaType::INTEGER>>(1920));
        m_config.addEntry("init_window_height",
                          std::make_unique<IRScript::LuaValue<IRScript::LuaType::INTEGER>>(1080));
        m_config.addEntry("game_resolution_width",
                          std::make_unique<IRScript::LuaValue<IRScript::LuaType::INTEGER>>(1920));
        m_config.addEntry("game_resolution_height",
                          std::make_unique<IRScript::LuaValue<IRScript::LuaType::INTEGER>>(1080));
        m_config.addEntry("fit_mode",
                          std::make_unique<IRScript::LuaValue<IRScript::ENUM, IRRender::FitMode>>(
                              IRRender::FitMode::FIT, [](const std::string &enumString) {
                                  if (enumString == "fit")
