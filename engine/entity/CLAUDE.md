...
references to the dying id. Two in-tree users: the modifier framework's
auto-sweep of source-attributed modifiers off live target entities
(`IRPrefab::Modifier::removeBySource`), and the voxel-pool canvas
...
- **Dangling EntityIds.** A surviving singleton/persistent entity holding the
  ...
  only what someone registered a hook for — today `C_Modifiers.source_` and
  `C_VoxelSetNew.canvasEntity_`. Every other id field is on its own: re-acquire
  after the next scene builds, or register a pre-destroy hook of your own.
