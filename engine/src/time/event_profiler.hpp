 ...
 * File: \irreden-engine\engine\src\time\event_profiler.hpp
    ...
    public:
        ...
        void endEvent() {
            ...
            if(MilliDuration(newTick).count() > kUpdateTimeWarningThresholdMs) {
                ENG_LOG_WARN("Update took {} ms", MilliDuration(newTick).count());
            }
            ...
            if(m_tickCount % 6000 == 0) {
                ENG_LOG_DEBUG("Update FPS: {} ms", m_sum.count() / 10.0f);

                MilliDuration totalElapsedTime = current - m_start;
                ENG_LOG_INFO("Average fixed updates per second: {} updates",
    ...
    public:
        ...
        void endEvent() {
            ...
            if(m_tickCount % 1000 == 0) {
                ENG_LOG_DEBUG("Render FPS: {} ms", m_sum.count() / (float)kProfileHistoryBufferSize);

                MilliDuration totalElapsedTime = current - m_start;
                ENG_LOG_INFO("Average render calls per second: {} updates",
                    m_tickCount * 1000.0f / totalElapsedTime.count()
                );
            }
        }
