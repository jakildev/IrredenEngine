...
#include "..\components\component_viewport.hpp"
#include "..\components\component_cursor_position.hpp"
#include "..\components\component_position_3d.hpp"
#include "..\components\component_camera.hpp"

#include "..\ecs\system_manager.hpp"

#include "system_input_key_mouse.hpp"
...
namespace IRECS {
    ...
    public:
        ...
        void beginExecuteRender() {
            if(m_isWheelClicked) {
                m_tempCameraOffset =
                    global.systemManager_->get<INPUT_KEY_MOUSE>()->getMousePositionRender().pos_ -
                    m_mouseWheelClickedStart;
            }
            m_mousePositionRenderTriangles =
                (
                    global.systemManager_->get<INPUT_KEY_MOUSE>()
        ...
        void zoomIn() {
            ENG_LOG_INFO("Zooming in");
            m_camera.get<C_Camera>().zoomIn();
        }

        void zoomOut() {
            ENG_LOG_INFO("Zooming out");
            m_camera.get<C_Camera>().zoomOut();
        }

        void dragCanvasStart() {
            m_mouseWheelClickedStart =
                global.systemManager_->get<INPUT_KEY_MOUSE>()->getMousePositionUpdate().pos_;
