...
class CommandManager {
  public:
    ...
    int registerMidiNoteCommand(int device, InputTypes InputType, Function command,
                                Args... fixedArgs) {
        if (!m_midiCCDeviceCommands.contains(device)) {
            m_midiNoteDeviceCommands.emplace(device,
    ...
    template <typename Function>
    int registerMidiCCCommand(int device, InputTypes InputType, unsigned char ccMessage,
                              Function command) {
        if (!m_midiCCDeviceCommands.contains(device)) {
    ...
    void executeDeviceMidiNoteCommands(int device,
