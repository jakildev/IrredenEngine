...
class SystemManager {
  public:
    ...
    SystemId createSystem(std::string name, FunctionTick functionTick,
  ...
  private:
    ...
    void insertBeginTickFunction(FunctionBeginTick functionBeginTick) {
        if constexpr (std::is_invocable_v<FunctionBeginTick>) {
            m_beginTicks.emplace_back(
                C_SystemEvent<BEGIN_TICK>{[functionBeginTick]() { functionBeginTick(); }});
        } else {
    ...
    template <typename... Components, typename... RelationComponents, typename FunctionTick>
    void insertTickFunction(FunctionTick functionTick,
                            RelationParams<RelationComponents...> extraParams) {
            ...
            [functionTick, extraParams](ArchetypeNode *node) {
                if constexpr (InvocableWithEntityId<FunctionTick, Components...>) {
                    auto componentsTuple = std::make_tuple(
                        std::ref(node->entities_), std::ref(getComponentData<Components>(node))...);
                    for (int i = 0; i < node->length_; i++) {
                        std::apply([i, &functionTick](
                                       auto &&...components) { functionTick(components[i]...); },
                                   componentsTuple);
                    }
                ...
                } else if constexpr (std::is_invocable_v<FunctionTick, Components &...,
                    ...
                    for (int i = 0; i < node->length_; i++) {
                        std::apply([&functionTick](auto &&...args) { functionTick(args...); },
                                   std::tuple_cat(
                                       std::make_tuple(std::ref(std::get<std::vector<Components> &>(
                                           componentsTuple)[i])...),
                                       relationComponentTuple));
                    }
                } else if constexpr (InvocableWithNodeVectors<FunctionTick, Components...>) {
                    auto paramTuple =
                        std::make_tuple(node->type_, std::ref(node->entities_),
    ...
    void insertEndTickFunction(FunctionEndTick functionEndTick) {
        if constexpr (std::is_invocable_v<FunctionEndTick>) {
            m_endTicks.emplace_back(
                C_SystemEvent<END_TICK>{[functionEndTick]() { functionEndTick(); }});
        } else {
        ...
        if constexpr (std::is_invocable_v<FunctionRelationTick, RelationComponents &...>) {
            m_relationTicks.emplace_back(
                C_SystemEvent<RELATION_TICK>{[functionRelationTick](EntityRecord entityRecord) {
                    auto componentsTuple =
                        std::make_tuple(std::ref(getComponentData<RelationComponents>(
                            entityRecord.archetypeNode)[entityRecord.row])...);
                    std::apply([functionRelationTick](
                                   auto &&...components) { functionRelationTick(components...); },
                               componentsTuple);
                }});
        } else if constexpr (std::is_same_v<FunctionRelationTick, std::nullptr_t>) {
            m_relationTicks.emplace_back(
                C_SystemEvent<RELATION_TICK>{[](EntityRecord record) { return; }});
        } else {
            static_assert(false, "Unsupported relation tick function signature.");
        }
    }

    EntityId handleRelationTick(ArchetypeNode *currentNode, SystemId currentSystem,
