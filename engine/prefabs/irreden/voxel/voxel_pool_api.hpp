...
// Notify position or alpha writes to [startIndex, startIndex + count).
// Hidden sets need this bounds-only path because visibility suppresses
// active-mask writes without suppressing the authored alpha used by bounds.
inline void
