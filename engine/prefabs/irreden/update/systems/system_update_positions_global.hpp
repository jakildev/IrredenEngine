...
template <> struct System<GLOBAL_POSITION_3D> {
    static SystemId create() {
        ...
        return createSystem<C_Position3D, C_PositionGlobal3D>(
            ...
            [](C_Position3D &position, C_PositionGlobal3D &positionGlobal,
               const std::optional<C_PositionGlobal3D *> parentPositionGlobal) {
                static C_PositionGlobal3D defaultParentPosition = {0, 0, 0};
                positionGlobal.pos_ =
                    position.pos_ + parentPositionGlobal.value_or(&defaultParentPosition)->pos_;
            },
            nullptr, nullptr, RelationParams<C_PositionGlobal3D>{Relation::CHILD_OF});
    }
};
