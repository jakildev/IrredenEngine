...
template <> struct System<TRIXEL_TO_FRAMEBUFFER> {
    static SystemId create() {
        ...
        return createSystem<C_TriangleCanvasTextures>(
            "CanvasToFramebuffer",
            ...
            nullptr,
            RelationParams<C_TrixelCanvasFramebuffer, C_FrameDataTrixelToFramebuffer, C_ZoomLevel>{
                Relation::CHILD_OF},
  ...
  private:
    static mat4 calcProjectionMatrix(const vec2 &resolution) {
        mat4 projection = glm::ortho(0.0f, resolution.x, 0.0f, resolution.y, -1.0f, 100.0f);
        return projection;
    }

    static mat4 calcModelMatrix(const vec2 &resolution, const vec2 &cameraPositionIso,
                                const vec2 &zoomLevel) {
        vec2 isoPixelOffset = glm::floor(IRMath::pos2DIsoToPos2DGameResolution(
                                  glm::fract(cameraPositionIso), zoomLevel)) *
                              vec2(1, -1);
        mat4 model = mat4(1.0f);
        model = glm::translate(model, glm::vec3(resolution.x / 2 + isoPixelOffset.x,
                                                resolution.y / 2 + isoPixelOffset.y, 0.0f));
        model = glm::scale(model,
                           glm::vec3(resolution.x * zoomLevel.x, resolution.y * zoomLevel.y, 1.0f));
        return model;
    }
};
