...
namespace IRScript {
    template <>
    inline constexpr bool kHasLuaBinding<IRComponents::C_Position3D> = true;

    template <>
    inline void bindLuaType<IRComponents::C_Position3D>(LuaScript& luaScript) {
        using IRComponents::C_Position3D;
        luaScript.registerType<
            C_Position3D,
            C_Position3D(float, float, float),
            C_Position3D(IRMath::vec3)
        >("C_Position3D",
