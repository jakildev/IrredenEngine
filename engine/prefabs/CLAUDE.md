...
- **Math primitives go through IRMath** — never `glm::*` or `std::sin/cos/sqrt/abs/min/max/clamp` directly; see [`.claude/rules/cpp-math.md`](../../.claude/rules/cpp-math.md).
...
- **No per-entity `getComponent` inside a system tick** — add the component to template params instead; see [`.claude/rules/cpp-ecs.md`](../../.claude/rules/cpp-ecs.md).
