...
void registerLuaBindings() {
    ...
    IREngine::registerLuaBindings([](IRScript::LuaScript &luaScript) {
        ...
        vec3Type[sol::meta_function::multiplication] =
            sol::overload([](const vec3 &a, float b) { return a * b; },
        ...
        luaScript.registerEnum<IRMidiNote>(
            "MidiNote", {{"A0", NOTE_A0},   {"Bb0", NOTE_A0_SHARP}, {"B0", NOTE_B0},
        ...
        luaScript.registerCreateEntityBatchFunction<C_Position3D, C_VoxelSetNew, C_PeriodicIdle,
                                                    C_MidiNote, C_ParticleBurst>(
            "createEntityBatchPolyrhythmBurst");

        // Batch creation: collision-driven note blocks.
        luaScript.registerCreateEntityBatchFunction<C_Position3D, C_VoxelSetNew, C_PeriodicIdle,
                                                    C_ColliderIso3DAABB, C_CollisionLayer,
                                                    C_ContactEvent, C_MidiNote, C_ParticleBurst,
                                                    C_TriggerGlow>(
            "createEntityBatchNoteBlocks");

        // Batch creation: static note platforms with spring visual behavior.
        luaScript.registerCreateEntityBatchFunction<C_Position3D, C_VoxelSetNew, C_ColliderIso3DAABB,
                                                    C_CollisionLayer, C_ContactEvent, C_Velocity3D,
                                                    C_ReactiveReturn3D>(
            "createEntityBatchNotePlatforms");
        ...
        luaScript.lua()["IRVisual"]["setMainCanvasPulseBackground"] =
            [](const Color &colorA, const Color &colorB, float pulseSpeed, int patternScale) {
                ...
                IREntity::setComponent(
                    backgroundCanvas,
                    C_TriangleCanvasBackground{BackgroundTypes::kPulsePattern, {colorA, colorB},
                                               size, pulseSpeed, patternScale});
            };
        luaScript.lua()["IRVisual"]["clearMainCanvasBackground"] = []() {
            IREntity::EntityId backgroundCanvas = IRRender::getCanvas("background");
            ivec2 size = IREntity::getComponent<C_SizeTriangles>(backgroundCanvas).size_;
            IREntity::setComponent(backgroundCanvas, C_TriangleCanvasBackground{
