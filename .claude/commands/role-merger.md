...
**You never merge PRs.** The fleet's only auto-merge lane — tier-0
plan-file merges in `fleet-rebase` — is dormant now that plans live as
issue comments and nothing lands under `.fleet/plans/**` (see FLEET.md
§ "Who merges"). Everything that reaches this LLM pass is
rebase/label/handoff work; never run `gh pr merge`.
...
- **Never `gh pr merge`.** Merging is the human's click (the tier-0
  plan-file auto-merge lane in `fleet-rebase` is dormant) — the LLM
