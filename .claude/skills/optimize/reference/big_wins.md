...
| Independent face-index work can use separate GPU lanes when allocation is already atomic and consumers wait for dispatch completion | `c_bake_box_sun_shadow`; [measurement](../../../../docs/perf/box-shadow-index-lanes.md) | Metal Debug large analytical box: capture-assisted mean frame 21.082 → 16.953 ms (19.6%); small cases show no wall-time win |
| One large face can dominate even after distributing faces; share one record and partition its tile reservations with explicit shared-slot publication and reuse barriers | `ir_sun_face_index`; [measurement](../../../../docs/perf/cooperative-box-face-index.md) | Metal Debug large analytical box: capture-assisted frame 16.790 → 9.580 ms (42.9%); small wall times essentially unchanged |
